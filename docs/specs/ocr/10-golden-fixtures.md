...
- `tests/test_ocr_fixtures.py` — `GOLDEN_SHA256`, `VLM_TO_GOLDEN_DIFFS` (99 → ожидаемо 100)
...
`VLM_TO_GOLDEN_DIFFS`. Ожидания: `VLM_TO_GOLDEN_DIFFS = 100` (+1 insert),
`REMAINING_DIFFS = 7`, сырой путь против нового эталона — **13**. Любое другое число —
стоп с распечаткой опкодов, не подгонять.
...
- `pytest -v` зелёный целиком; в `tests/test_ocr_fixtures.py` обновлены `GOLDEN_SHA256` и
  `VLM_TO_GOLDEN_DIFFS` (ожидаемо 100), в `tests/test_ocr_postprocess.py` — `REMAINING_DIFFS`
  (ожидаемо 7); отклонение от ожидаемых чисел объяснено в docs_sync поопкодно.
...
- В docs_sync — `### ПРАВКА #83` с таблицей
  ...
  по опкодам (таблица из диагностики 21.09.2026), разложение `VLM_TO_GOLDEN_DIFFS` от 79 к
  100 по правкам #74–#80 и правке 11 (как сделано для #68 и #72), и заметка для этапа C:
